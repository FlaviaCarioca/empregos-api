...
describe User, type: :model do
  ...
  it 'should find a user by correct credentials' do
    user = FactoryGirl.create(:user_candidate)

    fetched_user = User.find_by_credentials(user.email, user.password)

    expect(fetched_user.first.id).to eq user.id
  end

  it 'should not find a user if incorrect username is passed' do
    user = FactoryGirl.create(:user_candidate)

    fetched_user = User.find_by_credentials(Faker::Internet.email, user.password)

    expect(fetched_user).to be_empty
  end

  it 'should not find a user if incorrect password is passed' do
    user = FactoryGirl.create(:user_candidate)
  ...
  it 'should genereate a token if a user has correct credentials' do |variable|
    user = FactoryGirl.create(:user_candidate)

    fetched_user = User.find_by_credentials(user.email, user.password)

    token = fetched_user.first.generate_auth_token
